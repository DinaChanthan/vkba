...
def signup(request):
    ...
    if form.is_valid():
        ...
        if confirm_password != password:
            messages.error(request, 'Your confirm password does not match')
        else:
            user = auth.authenticate(
                request=request, email=email, password=password)
            if user:
                auth.login(request, user)
            messages.success(request, _('User has been created in VKBA System'))
...
def get_or_process_name_form(request):
    print("RRRRRRRRRRRRRRRRRR request",request,type(request))
    form = NameForm(data=request.POST or None, instance=request.user)
    print("FFFFFFFFFFFFFFFFFFFFFFF : form : ",form,type(form))
    if form.is_valid():
        form.save()
        messages.success(request, pgettext(
