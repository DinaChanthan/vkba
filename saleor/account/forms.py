...
class ChangePasswordForm(django_forms.PasswordChangeForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['old_password'].widget.attrs['placeholder'] = 'Your Old Passoword'
...
class SignupForm(forms.ModelForm, FormWithReCaptcha):
    ...
    confirm_password = forms.CharField(widget=forms.PasswordInput,
                                       label=pgettext('Confirm Password', 'Confirm Password'))
    first_name = forms.CharField(widget=forms.TextInput, label=pgettext('First Name', 'First Name'))
    last_name = forms.CharField(widget=forms.TextInput, label=pgettext('Last Name', 'Last Name'))
    phone_number = forms.DecimalField(widget=forms.NumberInput, label=pgettext('Phone Number', 'Phone Number'))
...
class NameForm(forms.ModelForm):
    class Meta:
        model = User
        # <EDIT & ADD
        fields = ['profile_image','first_name', 'last_name', 'phone_number','email',]
        labels = {
            'avatar': pgettext_lazy('Customer form: Image Field', 'Profile Image'),
            ...
            'phone_number': pgettext('Customer form: Phone number field', 'Phone Number'),
            'Email': pgettext('Customer form: Email field', 'Email')}
